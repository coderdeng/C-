...
void talk_to_client::do_login(const std::string &msg)
{
	...
	std::cout << "login user: " + _username << std::endl;
}
//处理注册请求
...
void talk_to_client::do_sendto(const std::string &msg)
{
	...
	if (to_user == _username){
		...
		clients.at(to_user)->do_send(requests.request_send_from + _username + " " + send_msg + "\n");
...
void talk_to_client::do_exit()
{
	if (clients.find(_username) == clients.end()){
		std::cout << "no user: " +  _username << std::endl;
		do_write(requests.request_exit + "false\n");
		return ;
	}
	clients.erase(clients.find(_username));
	for (std::map<std::string, client_ptr>::iterator it = clients.begin();
		it != clients.end(); ++it){
		it->second->set_chaged_clients();
	}
	std::cout << "logout user: " + _username << std::endl;
...
void talk_to_client::on_send(const error_code &err, size_t bytes)
{
	//do nothing
	if (!err){
		std::cout << "send to succeed\n" << std::endl;
		//do_write(requests.request_send_to + "succeed\n");
	}
	else 
